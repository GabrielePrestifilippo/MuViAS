define(function () {
        var NDVIPanel = function (wwd, layerManager) {
            ...
            var self = this;
            this.index = 0;
            $("#loadNDVIBtn").on("click", function () {
                self.addSurface(self.wwd, layerManager);
                $("#resetNDVIBtn").show();
                $("#loadNDVIBtn").hide();

            });

            $("#resetNDVIBtn").on("click", function () {
                for (var l in self.NDVILayers) {
                    wwd.removeLayer(self.NDVILayers[l]);
                }
                $("#graph").hide();
                $("#resetNDVIBtn").hide();
                $("#loadNDVIBtn").show();
        ...
        NDVIPanel.prototype.addSurface = function (wwd, layerManager) {

            var self = this;

            //var NDVILayer = new WorldWind.RenderableLayer("NDVI");
            //NDVILayer.index = this.index++;
            // this.myNDVI[this.index] = NDVILayer;
            var resourcesUrl = document.getElementById("NDVITxtArea").value;
            var dataJsonUrl = resourcesUrl;
            ...
            function addLoadingStatusParagraph(paragraphId, regionName, loadingStatus, totalNoOfScenes) {
                ...
                newParagraph.textContent = regionName + ": " + loadingStatus + "/" + totalNoOfScenes;
                ...
                worker.onmessage = function (e) {
                    ...
                    if (sceneCounter === analyticalSurfaceDataObject.length && !isFirstSceneDraw) {
                        ...
                        for (var i = 0; i < surfaceLayerArrayGlobal.length; i++) {
                            ...
                            self.NDVILayers.push(surfaceLayerArrayGlobal[i]);
        ...
        NDVIPanel.prototype.createInterface = function (wwd) {
            $("#NDVIList").html("");
            var self = this;
            for (var key in self.myNDVI) {
                var name = self.myNDVI[key].displayName + " " + key;
                var myDiv = $("<div key=" + key + " class='listJson'>&#10060;" + name + "</div>");
                $("#NDVIList").append(myDiv);
                myDiv.on('click', function () {
                    var myKey = $(this).attr("key");
                    wwd.removeLayer(self.myNDVI[myKey]);
                    $(this).remove();
                    delete(self.myNDVI[myKey]);
                })
